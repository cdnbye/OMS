<template>
  <div class="login-container">
    <el-form ref="signupForm" :model="signupForm" :rules="signupRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <el-button id="sendButton" :loading="sendLoading" type="primary" style="width:100%;margin-bottom:30px;" @click.native.prevent="onSendCode">{{ $t('signup.send') }}</el-button>
      ...
      <el-row type="flex" justify="space-between">
        <a @click="goLogin" style="color:#eee">登陆</a>
  ...
  methods: {
    ...
    handleSignup() {
      this.$refs.signupForm.validate(valid => {
        if (valid) {
          this.loading = true
          this.$store.dispatch('LoginByUsername', this.signupForm).then(() => {
            this.loading = false
